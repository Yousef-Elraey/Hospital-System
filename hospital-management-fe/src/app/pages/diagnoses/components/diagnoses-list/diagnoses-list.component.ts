...
export class DiagnosesListComponent implements OnInit {
  ...
  load(): void {
    this.loading = true;
    this.diagnoseService.getDiagnoses(toPageRequest(this.currentPage, this.pageSize)).subscribe({
      next: (response) => {
  ...
  delete(d: DiagnoseResponse): void {
    if (!d.id) return;
    this.confirmDialog
      .ask({ titleKey: 'confirm.title', messageKey: 'confirm.deleteDiagnose' })
      .subscribe((ok) => {
        if (!ok) return;
        this.diagnoseService.deleteDiagnose(d.id!).subscribe({
          next: () => this.load(),
        });
      });
  }
